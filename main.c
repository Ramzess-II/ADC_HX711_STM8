...
uint16_t weight_push = 2000;                        // вес при котором идет запуск  
  ...
  if (!PC_IDR_IDR3) {                             // если нажата кнопка калибровки
    ...
    while (1) {                                       // ждем калибровки веса
      ...
      if (!PC_IDR_IDR3 && flag.pin_but_reset) {       // если нажали кнопку и кнопка была отпущена
        new_kalib_koef (CAL_MASS);                    // рассчитаем новый калибровочный коэф 
        set_buzer (5, 5000);
        delay_ms(3000);
        break;
      }
...
void check_beep (void){                           // проверка не нужно ли пикать
  if (abs (massa_int) > (weight_push / DEVIDER_MASS)  &&  massa_int < weight_push) {                      // если масса больше веса запуска деленного на делитель и меньше чем вес запуска
   set_buzer (1, map (abs (massa_int), (weight_push / DEVIDER_MASS), weight_push, MIN_FREQ, MAX_FREQ));   // пикать с частотой относительно минимальной и максимальной заданной
  } 
  if (massa_int >= weight_push && !flag.beep_start) {  // если масса больше или равна весу отпускания и флаг единоразового запуска бипера опущен ( бипер не работает если его постоянно запускать) 
...
__interrupt void TIM1_OVR_UIF_Handler (void) {             // обработчик прерываний от таймера 1
    ...
    if (tim_to_update_ds18b20 == 32767) flag.read_sensor = true;  // если 32767 запускаем считывание
